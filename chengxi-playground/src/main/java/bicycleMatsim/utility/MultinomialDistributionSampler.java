...
public class MultinomialDistributionSampler implements RandomDistributionSampler {
	...
	public Integer[] sampleWitReplacement(int n) {
	...
	public Integer[] sampleWitoutReplacement(int n) {
		...
		if (n>this.sum) {
			...
			while (i<=n) {
				...
				// (2) check if the corresponding weight is over 1, if yes, flag returns true, otherwise flag returns false
				boolean flag =true;
				if (trialIndex==0 && cdfWeightShrinking[trialIndex]<1) {
					flag=false;
				} else if (trialIndex>0 && trialIndex < (cdfWeightShrinking.length) && cdfWeightShrinking[trialIndex]-cdfWeightShrinking[trialIndex-1]<1) {
					 flag =false;
				...
				// (3) if the corresponding weight is over one, substract one from the cdfWeightShrinking and sumShrinking
				// otherwise do another trial sample
				if(flag==true) {
